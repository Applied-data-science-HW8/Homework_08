...
def cut_file(infile, outfile, delimiter=',', keep_list=None):
    ...
    csvwriter=csv.writer(outfile,delimiter=delimiter) 
    ...
    if keep_list==None:
        keep_list=[]
    csvwriter.writerow(keep_list)

    ## Get the indices in the file that we will keep
    indices=[]
    for keepcolumn in keep_list:
        i=0
    ...
    for row in csvreader:
        reformatrow=[]
        for i in indices:
            reformatrow.append(row[i])
        csvwriter.writerow(reformatrow)
