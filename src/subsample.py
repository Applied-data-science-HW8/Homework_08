...
import pdb
import math
import homework_08.src.common as common
...
def subsample(infile, outfile, subsample_rate=0.01, subsample_number=1, delimiter=',', seed=None):
    """
    ...
    if subsample_number != 1:
        subsample_idx =set(random.sample(range(totalrows-1), subsample_number))
    else:
        subsample_idx =set(random.sample(range(totalrows-1), int(totalrows*subsample_rate)))
